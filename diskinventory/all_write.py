...
"""
   reads sqllite databases created by readlist.py  and readdu.py and writes out
    file information to a pandas dataframe

   example:  python ~/repos/pythonlibs/diskinventory/all_write.py files_pha.db scm16_pha.h5
"""
...
parser.add_argument('file_db',type=str,help='file list db  generated by read_cron.py')
parser.add_argument('out_h5',type=str,help='HDFstore output file')
parser.add_argument('--size',nargs='?',default=0, type=float,help='float smallest size in bytes (default 0)')
args=parser.parse_args()


dbstring='sqlite:///{:s}'.format(args.file_db)
...
large_query=thesession.query(file_table).filter(file_table.c.size > args.size).with_labels()
...
with pd.HDFStore(args.out_h5,'w') as store:
    store.put('files',saveit,format='table')
